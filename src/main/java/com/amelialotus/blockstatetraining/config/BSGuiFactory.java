...
public class BSGuiFactory implements IModGuiFactory
{
  ...
  {
    return new BSConfigGui(parentScreen);
  }
  ...
  //This class inherits from GuiConfig, a specialized GuiScreen designed to display your
	...
	public static class BSConfigGui extends GuiConfig
	{
		public BSConfigGui(GuiScreen parentScreen)
		{
			//I18n function basically "translates" or localizes the given key using the appropriate .lang file
			super(parentScreen, getConfigElements(), MainMod.MODID,
		...
		private static List<IConfigElement> getConfigElements()
		{
			...
			list.add(new DummyCategoryElement("General", "gui.config.ctgy.general", CategoryEntryGeneral.class));
			list.add(new DummyCategoryElement("Other", "gui.config.ctgy.other", CategoryEntryOther.class));
			return list;
		}
        ...
        Configuration configuration = configInit.getConfig();
        ConfigElement cat_general = new ConfigElement(configuration.getCategory(configInit.CATEGORY_NAME_GENERAL));
        ...
        return new GuiConfig(this.owningScreen, propertiesOnThisScreen,
                              ...
                              configInit.CATEGORY_NAME_GENERAL,
		...
		public static class CategoryEntryOther extends CategoryEntry
		{
			...
			{
        Configuration configuration = configInit.getConfig();
        ConfigElement cat_general = new ConfigElement(configuration.getCategory(configInit.CATEGORY_NAME_OTHER));
        List<IConfigElement> propertiesOnThisScreen = cat_general.getChildElements();
        String windowTitle = configuration.toString();

        return new GuiConfig(this.owningScreen, propertiesOnThisScreen,
                             this.owningScreen.modID,
                             configInit.CATEGORY_NAME_OTHER,
                             this.configElement.requiresWorldRestart() || this.owningScreen.allRequireWorldRestart,
                             this.configElement.requiresMcRestart() || this.owningScreen.allRequireMcRestart,
                             windowTitle);
        //this is a complicated object that specifies the category's gui screen, to better understand
